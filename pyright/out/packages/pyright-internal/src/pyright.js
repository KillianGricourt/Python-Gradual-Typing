...
const chokidarFileWatcherProvider_1 = require("./common/chokidarFileWatcherProvider");
...
var ExitStatus;
(function (ExitStatus) {
    ExitStatus[ExitStatus["NoErrors"] = 0] = "NoErrors";
    ExitStatus[ExitStatus["ErrorsReported"] = 1] = "ErrorsReported";
    ExitStatus[ExitStatus["FatalError"] = 2] = "FatalError";
    ExitStatus[ExitStatus["ConfigFileParseError"] = 3] = "ConfigFileParseError";
    ExitStatus[ExitStatus["ParameterError"] = 4] = "ParameterError";
...
async function processArgs() {
    ...
    let args;
    try {
        args = (0, command_line_args_1.default)(optionDefinitions);
    }
    catch (e) {
        const argErr = e;
    ...
    const options = new commandLineOptions_1.CommandLineOptions(process.cwd(), false);
    ...
    if (args.files && Array.isArray(args.files)) {
        ...
        options.includeFileSpecsOverride = options.includeFileSpecsOverride.map((f) => (0, pathUtils_1.combinePaths)(process.cwd(), f));
        // Verify the specified file specs to make sure their wildcard roots exist.
        const tempFileSystem = new pyrightFileSystem_1.PyrightFileSystem((0, realFileSystem_1.createFromRealFileSystem)(tempFile));
        for (const fileDesc of options.includeFileSpecsOverride) {
            const includeSpec = (0, uriUtils_1.getFileSpec)(uri_1.Uri.file(process.cwd(), tempFile), fileDesc);
            try {
                const stat = (0, uriUtils_1.tryStat)(tempFileSystem, includeSpec.wildcardRoot);
                if (!stat) {
    ...
    if (args.project) {
        options.configFilePath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args.project));
    }
    ...
    if (args.pythonversion) {
        const version = pythonVersion_1.PythonVersion.fromString(args.pythonversion);
        if (version) {
    ...
    if (args.pythonpath !== undefined) {
        ...
        options.pythonPath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args['pythonpath']));
    }
    if (args['venv-path']) {
        console.warn(`'venv-path' option is deprecated; use 'venvpath' instead`);
        options.venvPath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args['venv-path']));
    }
    if (args['venvpath']) {
        options.venvPath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args['venvpath']));
    }
    if (args['typeshed-path']) {
        console.warn(`'typeshed-path' option is deprecated; use 'typeshedpath' instead`);
        options.typeshedPath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args['typeshed-path']));
    }
    if (args['typeshedpath']) {
        options.typeshedPath = (0, pathUtils_1.combinePaths)(process.cwd(), (0, pathUtils_1.normalizePath)(args['typeshedpath']));
    }
    if (args.createstub) {
    ...
    let minSeverityLevel = 'information';
    ...
    let logLevel = console_1.LogLevel.Error;
    if (args.stats || args.verbose) {
    ...
    const fileSystem = new pyrightFileSystem_1.PyrightFileSystem((0, realFileSystem_1.createFromRealFileSystem)(tempFile, output, new chokidarFileWatcherProvider_1.ChokidarFileWatcherProvider(output)));
    const serviceProvider = (0, serviceProviderExtensions_1.createServiceProvider)(fileSystem, output, tempFile);
    ...
    const service = new service_1.AnalyzerService('<default>', serviceProvider, {
        console: output,
        hostFactory: () => new fullAccessHost_1.FullAccessHost(serviceProvider),
...
async function runSingleThreaded(args, options, service, minSeverityLevel, output) {
    const watch = args.watch !== undefined;
    const treatWarningsAsErrors = !!args.warnings;
    const exitStatus = (0, deferred_1.createDeferred)();
    service.setCompletionCallback((results) => {
        ...
        if (!args.createstub && !args.verifytypes) {
            if (args.outputjson) {
                const report = reportDiagnosticsAsJson(results.diagnostics, minSeverityLevel, results.filesInProgram, results.elapsedTime);
        ...
        if (!args.outputjson) {
            if (!watch) {
                ...
                timing_1.timingStats.printSummary(output);
            }
            if (args.stats) {
                // Print the stats details.
                service.printStats();
                timing_1.timingStats.printDetails(console);
                if (args.verbose) {
...
async function runMultiThreaded(args, options, maxThreadCount, service, minSeverityLevel, output) {
    const workers = [];
    ...
    const affinityQueues = new Array(workerCount);
    ...
    const sendMessageToWorker = (worker, message, data) => {
        worker.send(JSON.stringify({ action: message, data: data }));
    };
    const analyzeNextFile = (workerIndex) => {
        const worker = workers[workerIndex];
        let nextFileToAnalyze;
        ...
        if (nextFileToAnalyze) {
            ...
            if (pendingAnalysisCount === 0) {
                ...
                if (!exitStatus.resolved) {
                    ...
                    let errorCount = 0;
                    if (args.outputjson) {
                        const report = reportDiagnosticsAsJson(fileDiagnostics, minSeverityLevel, sourceFilesToAnalyze.length, elapsedTime);
                        errorCount += report.errorCount;
                        if (treatWarningsAsErrors) {
                            errorCount += report.warningCount;
                        }
    ...
    for (let i = 0; i < workerCount; i++) {
        ...
        const worker = (0, child_process_1.fork)(mainModulePath, ['worker', i.toString()]);
            ...
            switch (messageObj.action) {
                case 'analysisResults': {
                    ...
                    for (const fileDiag of results.diagnostics) {
                        fileDiagnostics.push(diagnosticSink_1.FileDiagnostics.fromJsonObj(fileDiag));
                    }
                    analyzeNextFile(i);
                    break;
                }
...
function runWorkerMessageLoop(workerNum) {
    ...
    const sendMessageToParent = (message, data) => {
        var _a;
        (_a = process.send) === null || _a === void 0 ? void 0 : _a.call(process, JSON.stringify({ action: message, data: data }));
    };
        ...
        switch (messageObj.action) {
            case 'setOptions': {
                ...
                Object.keys(messageObj.data).forEach((key) => {
                    options[key] = messageObj.data[key];
                });
                let logLevel = console_1.LogLevel.Error;
                ...
                service.setCompletionCallback((results) => {
                    ...
                    const fileDiags = results.diagnostics.filter((fileDiag) => fileDiag.fileUri.equals(lastOpenFileUri));
                    // Convert JSON-compatible format.
                    const resultsObj = {
                        ...results,
                        diagnostics: fileDiags.map((fileDiag) => diagnosticSink_1.FileDiagnostics.toJsonObj(fileDiag)),
                    };
            ...
            case 'analyzeFile': {
                if (serviceProvider && fileSystem && service) {
                    const uri = uri_1.Uri.parse(messageObj.data, serviceProvider);
                    // Check the file's length before attempting to read its full contents.
                    const fileStat = fileSystem.statSync(uri);
                    if (fileStat.size > sourceFile_1.maxSourceFileSize) {
                        console.error(`File length of "${uri}" is ${fileStat.size} ` +
                            `which exceeds the maximum supported file size of ${sourceFile_1.maxSourceFileSize}`);
                        throw new Error('File larger than max');
                    }
                    const fileContents = fileSystem.readFileSync(uri, 'utf8');
                    lastOpenFileUri = uri;
                    service === null || service === void 0 ? void 0 : service.setFileOpened(uri, /* version */ 1, fileContents);
                }
                break;
            }
        }
...
function verifyPackageTypes(serviceProvider, packageName, options, outputJson, minSeverityLevel, ignoreUnknownTypesFromImports) {
    try {
        const host = new fullAccessHost_1.FullAccessHost(serviceProvider);
        const verifier = new packageTypeVerifier_1.PackageTypeVerifier(serviceProvider, host, options, packageName, ignoreUnknownTypesFromImports);
...
function accumulateReportDiagnosticStats(diag, report) {
...
function buildTypeCompletenessReport(packageName, completenessReport, minSeverityLevel) {
    var _a, _b, _c;
    const report = {
        ...
        summary: {
            ...
            warningCount: 0,
            informationCount: 0,
            timeInSec: timing_1.timingStats.getTotalDuration(),
        },
    };
    ...
    report.typeCompleteness = {
        packageName,
        packageRootDirectory: (_a = completenessReport.packageRootDirectoryUri) === null || _a === void 0 ? void 0 : _a.getFilePath(),
        moduleName: completenessReport.moduleName,
        moduleRootDirectory: (_b = completenessReport.moduleRootDirectoryUri) === null || _b === void 0 ? void 0 : _b.getFilePath(),
        ignoreUnknownTypesFromImports: completenessReport.ignoreExternal,
        pyTypedPath: (_c = completenessReport.pyTypedPathUri) === null || _c === void 0 ? void 0 : _c.getFilePath(),
    ...
    completenessReport.symbols.forEach((symbol) => {
        ...
        const jsonSymbol = {
            category: packageTypeVerifier_1.PackageTypeVerifier.getSymbolCategoryString(symbol.category),
            name: symbol.fullName,
            referenceCount: symbol.referenceCount,
            isExported: symbol.isExported,
            isTypeKnown: symbol.typeKnownStatus === 0 /* TypeKnownStatus.Known */,
            isTypeAmbiguous: symbol.typeKnownStatus === 1 /* TypeKnownStatus.Ambiguous */,
            diagnostics,
        };
        ...
        if (symbol.typeKnownStatus === 0 /* TypeKnownStatus.Known */) {
        ...
        else if (symbol.typeKnownStatus === 1 /* TypeKnownStatus.Ambiguous */) {
            if (symbol.isExported) {
...
function printTypeCompletenessReportText(results, verboseOutput) {
    ...
    // Print other stats.
    console.info('');
    console.info(`Symbols exported by "${completenessReport.packageName}": ${completenessReport.exportedSymbolCounts.withKnownType +
    ...
    console.info('');
    console.info(`Other symbols referenced but not exported by "${completenessReport.packageName}": ${completenessReport.otherSymbolCounts.withKnownType +
...
function reportDiagnosticsAsJson(fileDiagnostics, minSeverityLevel, filesInProgram, timeInSec) {
    const report = {
        version: getVersionString(),
        time: Date.now().toString(),
        generalDiagnostics: [],
    ...
    fileDiagnostics.forEach((fileDiag) => {
        fileDiag.diagnostics.sort(diagnostic_1.compareDiagnostics).forEach((diag) => {
            if (diag.category === 0 /* DiagnosticCategory.Error */ ||
...
function isDiagnosticIncluded(diagSeverity, minSeverityLevel) {
...
function convertDiagnosticCategoryToSeverity(category) {
    switch (category) {
        ...
        case 2 /* DiagnosticCategory.Information */:
            return 'information';
        default:
            (0, debug_1.fail)('Unexpected diagnostic category');
    }
}
function convertDiagnosticToJson(filePath, diag) {
    return {
        file: filePath,
        severity: convertDiagnosticCategoryToSeverity(diag.category),
        message: diag.message,
        range: (0, textRange_1.isEmptyRange)(diag.range) ? undefined : diag.range,
        rule: diag.getRule(),
    };
}
function reportDiagnosticsAsText(fileDiagnostics, minSeverityLevel) {
    let errorCount = 0;
    let warningCount = 0;
    ...
    fileDiagnostics.forEach((fileDiagnostics) => {
        ...
        const fileErrorsAndWarnings = fileDiagnostics.diagnostics
            .filter((diag) => diag.category !== 3 /* DiagnosticCategory.UnusedCode */ &&
            ...
            .sort(diagnostic_1.compareDiagnostics);
        if (fileErrorsAndWarnings.length > 0) {
            ...
            fileErrorsAndWarnings.forEach((diag) => {
                ...
                else if (diag.category === 1 /* DiagnosticCategory.Warning */) {
                    warningCount++;
                }
                else if (diag.category === 2 /* DiagnosticCategory.Information */) {
                    informationCount++;
                }
            });
        }
    });
    console.info(`${errorCount.toString()} ${errorCount === 1 ? 'error' : 'errors'}, ` +
...
function logDiagnosticToConsole(diag, prefix = '  ') {
    ...
    if (diag.range && !(0, textRange_1.isEmptyRange)(diag.range)) {
        message +=
            chalk_1.default.yellow(`${diag.range.start.line + 1}`) +
                ':' +
                chalk_1.default.yellow(`${diag.range.start.character + 1}`) +
                ' - ';
    }
    else {
        message += ' ';
    ...
    message +=
        diag.severity === 'error'
            ...
            : diag.severity === 'warning'
                ? chalk_1.default.cyan('warning')
                : chalk_1.default.blue('information');
    message += `: ${firstLine}`;
    if (remainingLines.length > 0) {
        message += '\n' + prefix + remainingLines.join('\n' + prefix);
    }
    if (diag.rule) {
        message += chalk_1.default.gray(` (${diag.rule})`);
    }
    console.info(message);
}
function parseThreadsArgValue(input) {
