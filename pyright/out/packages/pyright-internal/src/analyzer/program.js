...
class Program {
    constructor(initialImportResolver, initialConfigOptions, serviceProvider, logTracker, _disableChecker, id) {
        ...
        this._console = serviceProvider.tryGet(serviceKeys_1.ServiceKeys.console) || new console_1.StandardConsole();
        ...
        this._cacheManager = (_a = serviceProvider.tryGet(serviceKeys_1.ServiceKeys.cacheManager)) !== null && _a !== void 0 ? _a : new cacheManager_1.CacheManager();
    ...
    getFileCount(userFileOnly = true) {
        if (userFileOnly) {
            return this._sourceFileList.filter((f) => (0, sourceFileInfoUtils_1.isUserCode)(f)).length;
        }
        ...
        this._sourceFileList.forEach((fileInfo) => {
            ...
            if (sourceFile.isCheckingRequired()) {
                if (this._shouldCheckFile(fileInfo)) {
                    sourceFile.getIPythonMode() === sourceFile_1.IPythonMode.CellDocs
                        ? cellsToAnalyzeCount++
                        : filesToAnalyzeCount++;
                }
            }
    ...
    getParserOutput(fileUri) {
        ...
        /* content */ undefined, 
        /* force */ true)) === null || _a === void 0 ? void 0 : _a.sourceFile.getParserOutput();
    }
    ...
    getTextOnRange(fileUri, range, token) {
        ...
        return this._runEvaluatorWithCancellationToken(token, () => {
            ...
            const textRange = (0, positionUtils_1.convertRangeToTextRange)(range, parseResults.tokenizerOutput.lines);
            if (!textRange) {
                return undefined;
            }
    ...
    getDiagnostics(options, reportDeltasOnly = true) {
        const fileDiagnostics = this._removeUnneededFiles();
        this._sourceFileList.forEach((sourceFileInfo) => {
            if (this._shouldCheckFile(sourceFileInfo)) {
                let diagnostics = sourceFileInfo.sourceFile.getDiagnostics(options, reportDeltasOnly ? sourceFileInfo.diagnosticsVersion : undefined);
                if (diagnostics !== undefined) {
                    // Filter out all categories that are translated to tagged hints?
                    if (options.disableTaggedHints) {
                        diagnostics = diagnostics.filter((diag) => diag.category !== 4 /* DiagnosticCategory.UnreachableCode */ &&
                            diag.category !== 3 /* DiagnosticCategory.UnusedCode */ &&
    ...
    emptyCache() {
        ...
        this._parsedFileCount = 0;
        (_a = this.serviceProvider.tryGet(serviceKeys_1.ServiceKeys.stateMutationListeners)) === null || _a === void 0 ? void 0 : _a.forEach((l) => { var _a; return (_a = l.onClearCache) === null || _a === void 0 ? void 0 : _a.call(l); });
    }
    ...
    _runEvaluatorWithCancellationToken(token, callback) {
        try {
            ...
            if (!cancellationUtils_1.OperationCanceledException.is(e) || e.isTypeCacheInvalid) {
                this._createNewEvaluator();
            }
            throw e;
        }
    ...
    _isImportAllowed(importer, importResult, isImportStubFile) {
        ...
        let thirdPartyImportAllowed = this._configOptions.useLibraryCodeForTypes ||
            (importResult.importType === 1 /* ImportType.ThirdParty */ && !!importResult.pyTypedInfo) ||
            (importResult.importType === 2 /* ImportType.Local */ && importer.isThirdPartyPyTypedPresent);
        if (importResult.importType === 1 /* ImportType.ThirdParty */ ||
            (importer.isThirdPartyImport && importResult.importType === 2 /* ImportType.Local */)) {
        ...
        while (nextImplicitImport) {
            ...
            if (implicitSet.has(implicitPath.key)) {
                ...
                debug.fail((_b = (_a = this.serviceProvider
                    .tryGet(serviceKeys_1.ServiceKeys.debugInfoInspector)) === null || _a === void 0 ? void 0 : _a.getCycleDetail(this, nextImplicitImport)) !== null && _b !== void 0 ? _b : `Found a cycle in implicit imports files`);
            }
    ...
    _bindFile(fileToBind, content, skipFileNeededCheck = false, isImplicitImport = false) {
        var _a;
        ...
        if (fileToBind.builtinsImport && fileToBind.builtinsImport !== fileToBind) {
            ...
            // the builtin module.
            builtinsScope =
                (_a = getScopeIfAvailable(fileToBind.chainedSourceFile)) !== null && _a !== void 0 ? _a : getScopeIfAvailable(fileToBind.builtinsImport);
        }
    ...
    _checkTypes(fileToCheck, token, chainedByList) {
        return this._logTracker.log(`analyzing: ${fileToCheck.sourceFile.getUri()}`, (logState) => {
            ...
            const boundFile = this._bindFile(fileToCheck, undefined, 
    ...
    _checkDependentFiles(fileToCheck, chainedByList, token) {
        ...
        chainedByList = chainedByList !== null && chainedByList !== void 0 ? chainedByList : (0, sourceFileInfoUtils_1.createChainedByList)(this, fileToCheck);
        ...
        for (let i = startIndex; i < chainedByList.length; i++) {
            ...
            const parseResults = file === null || file === void 0 ? void 0 : file.sourceFile.getParserOutput();
            if (!parseResults) {
                continue;
            }
